...
const teamResourceRequestScopeFromContext = (_context: any): any => null;
...
const shouldEmitWorkspaceBillingRuntimeNudge = (_state: any): boolean => false;
const teamResourceMaterializationDir = (..._args: any[]): string => '';
const resolveUserDesignSystemShareDirectory = (..._args: any[]): string => '';
...
const emitSharedProjectPullTiming = (..._args: any[]): void => {};
const handleHubTeamProjectsChanged = async (..._args: any[]): Promise<void> => {};
const handleHubProjectMetadataChanged = async (..._args: any[]): Promise<void> => {};
const teamProjectsForDisplay = async (_context: any): Promise<any[]> => [];
const teamMembersForDisplay = async (_context: any): Promise<any[]> => [];
const teamProjectsDisplayScopeFromContext = (_context: any): any => null;
const withoutLocallyUnsharedProjects = async <T>(projects: T[]): Promise<T[]> => projects;
const projectIsUnmaterializedSharedPlaceholder = (_projectId: string): boolean => false;
const teamResourceRequestScopeForWorkspaceId = (_items: any, _workspaceId: string): any => null;
const emitTeamProjectsChanged = (..._args: any[]): void => {};
const directoryConnectionKey = (_workspaceId: string, _identityKey: string): string => `${_workspaceId}:${_identityKey}`;
const syncWorkspaceDirectoryRealtimeHealth = (): void => {};
const currentWorkspaceDirectoryIdentity = (): string => '';
...
const createPersistentSyncCache = (_opts: any) => {
  const fn = async () => null;
  return Object.assign(fn, { invalidate: () => {} });
};
const createSwrCache = (_fetcher: any, _keyFn: any, _freshMs: number) => {
  const fn = async () => null;
  return Object.assign(fn, { invalidate: () => {} });
};
const createSyncDigestReader = (_opts: any) => async () => null;
const parseTeamProjectSnapshot = (_data: any) => null;
const parseMemberDirectorySnapshot = (_data: any) => null;
const createSqlitePublicFilePublicationStore = (_db: any) => ({}) as any;
const createRememberedTeamResourceScopes = () => ({
...
const API_TOKEN_BASIC_CHALLENGE = 'Basic realm="open-design"';
const apiTokenAuthorizationMatches = (_authorization: string | undefined, _token: string): boolean => false;
const apiTokenFromEnv = (): string => process.env.OD_API_TOKEN ?? '';
const isApiAuthDisabled = (): boolean => process.env.OD_DISABLE_API_AUTH === '1';
const isApiTokenMiddlewareEnabled = (): boolean => apiTokenFromEnv().length > 0 && !isApiAuthDisabled();
...
export async function startServer({
  ...
  // [REMOVED: workspace authority cache mode - deleted collab module]
  const workspaceAuthorityCacheMode = 'default' as any;
  ...
  const workspaceExactAuthorityCache = {
    cached: (_id: string) => null,
    observe: () => {},
    resetIdentity: () => {},
    invalidate: () => {},
    setRealtimeHealthy: () => {},
  } as any;
  const workspaceDirectoryAuthority = {
    read: async () => ({ ok: false as const, items: [] as any[] }),
    fresh: async () => ({ ok: false as const, items: [] as any[] }),
    backgroundFresh: async () => ({ ok: false as const, items: [] as any[] }),
    cached: () => ({ ok: false as const, items: [] as any[] }),
    resetIdentity: () => {},
    invalidate: () => {},
    setRealtimeHealthy: () => {},
  } as any;
  ...
  // [REMOVED: workspace hub subscriptions - deleted collab module]
  let workspaceHubSubscriptions: any = null;
  ...
  // [REMOVED: resolveAuthoritativeTeamWorkspaceContext - deleted collab module]
  ...
  async function resolveDesignSystemWorkspaceContext(
    req: any,
  ): Promise<{ workspaceId: string; [key: string]: unknown } | null> {
  ...
  const createWorkspaceOwnedDesignSystemForContext = (
    ...
    context: { workspaceId: string; [key: string]: unknown } | null,
  ...
  let workspaceHubAccountIdentity = '' as any;
  ...
  const fetchWorkspaceDirectoryForAccountSurface = () => fetchWorkspaceDirectory();
  ...
  // [REMOVED: verifyWorkspaceContextReadAuthority - deleted collab module]
  const verifyWorkspaceContextReadAuthority = async (_req: unknown) => {
    return { ok: false as const, status: 403 as const, code: 'WORKSPACE_ACCESS_DENIED' as const, message: 'workspace context unavailable' };
  };
  /**
  ...
  // [REMOVED: collab runtime creation - deleted collab module]
  const collab = {
  ...
  // [REMOVED: team share recovery - deleted collab module]
  // [REMOVED: reconcileImpossibleTeamShares - deleted collab module]
  ...
  // [REMOVED: collab cloud client, team projects lister, collab cloud service - deleted collab modules]
  const collabCloudClient = null as any;
  ...
  // [REMOVED: team projects lister - deleted collab module]
  const teamProjectsLister = async (_workspaceId: string) => [] as any[];

  // [REMOVED: collab cloud service - deleted collab module]
  const collabCloud = null as any;
  ...
  // [REMOVED: workspace projects reconciler - deleted collab module]
  const workspaceProjectsReconcilerDeps = (_requestedWorkspaceId: string): any => null;
  const reconcileWorkspaceProjectsFromRemote = (_requestedWorkspaceId: string): Promise<void> => Promise.resolve();
  const reconcileWorkspaceProjectMetadataFromRemote = (_requestedWorkspaceId: string, _projectId: string): Promise<void> => Promise.resolve();
  // [REMOVED: resolveSharedProject - deleted collab module]
  const resolveSharedProject = async (_projectId: string, _scope?: any) => null;
  // [REMOVED: resolveSharedProjectOwner - deleted collab module]
  ...
  // [REMOVED: collab presence routes - deleted collab module]
  const presenceRoutes = {
    invalidatePresence: () => {},
    markPresenceStale: () => {},
  } as any;
  // [REMOVED: collab publish watcher - deleted collab module]
  const collabPublishWatcher = { start: () => {} } as any;
  const sharedProjectPullProfiling = false;
  ...
  // [REMOVED: collab sync routes - deleted collab module]
  const collabSyncRoutes = {
    pullSharedProject: async () => false,
  } as any;
  // [REMOVED: remaining collab sync route deps - deleted collab module]
  ...
  // [REMOVED: proactive content pull and background pull size guard - deleted collab modules]
  const proactiveTeamProjectMaterializedVersion = (_target: any) => null;
  const backgroundPullSizeGuard = {
    assess: () => {},
    volume: () => ({ entries: 0, countedProjects: 0, uncountedProjects: 0 }),
  } as any;
  const proactiveContentPull = {
    materializeMissingProjects: async () => {},
    catchUpPublishedHeads: async () => {},
    handleContentChanged: async () => {},
    advanceRecoveryFloor: () => {},
    observeMaterialized: () => {},
  } as any;
  observeLegacyTeamProjectPull = () => {};
  // [REMOVED: team members cache - deleted collab module]
  const teamMembersCache = null as any;
  const teamMembersForDisplay = async (
    _context: any,
  ): Promise<any[]> => [];
  // [REMOVED: account billing summary cache - deleted collab module]
  // [REMOVED: workspace billing runtime - deleted collab module]
  const workspaceBillingRuntime = {
    reconnect: () => {},
    invalidate: () => {},
    interestedKeys: () => [],
    revokeWorkspace: () => {},
  ...
  // [REMOVED: workspace invalidation poller - deleted collab module]
  const workspaceInvalidationPollerFor = (_workspaceIdInput: string) => ({
    pollOnce: async () => {},
    setRealtimeHealthy: () => {},
  });
  const pollWorkspaceInvalidationForWorkspace = (
    _workspaceIdInput: string,
  ): Promise<void> => Promise.resolve();
  // [REMOVED: workspace authority health coordinator - deleted collab module]
  const workspaceAuthorityHealth = {
    update: async () => {},
  } as any;
