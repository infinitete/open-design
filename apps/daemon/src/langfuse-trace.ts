...
/**
 * Effective sink for completed-run and feedback telemetry. Delivery always
 * goes through the hosted relay or the direct Langfuse credentials; the
 * retired cloud provider's authenticated sink is no longer selectable.
 */
...
/**
 * Task hierarchy delivery resolves the same generic relay / direct sinks as
 * completed-run telemetry. Keep this named seam separate from
 * completed-run/feedback resolution so Task eligibility keeps its own knob.
 */
 ...
 * hosted relay first, direct Langfuse credentials second for local smoke
 * tests, disabled last.
 */
 ...
 * this helper owns only the existing Relay/direct wire behavior. Keeping
