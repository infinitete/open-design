...
async function executeProjectFilesSearch(options: ExecuteLocalDaemonRefreshSourceOptions): Promise<BoundedJsonObject> {
  ...
  const allFiles = await listFiles(options.projectsRoot, options.projectId, { metadata: options.projectMetadata }) as Array<{ name: string; path: string; type: string; size: number; mtime: number; kind?: string; mime?: string }>;
  ...
  for (const file of allFiles) {
    ...
    if (!matched && normalizedQuery !== undefined && isTextLikeFile(file) && file.size <= 128 * 1024) {
      try {
        const entry = await readProjectFile(options.projectsRoot, options.projectId, file.path, options.projectMetadata);
