...
//     uncached = input - read.
//   - ADDITIVE (Anthropic, and the Responses-API / ACP usage that the retired
//     cloud runtime's and pi's STREAM emits): input_tokens is the UNCACHED
//     remainder and the cache-read/creation tokens are reported separately on
//     top → effective = input + read + creation, uncached = input.
...
// `first_call_cache_hit_ratio` blow past 1.0 (observed on a large share of
// follow-up runs on both additive runtimes) and `uncached_input_tokens`
// collapses to 0.
//
