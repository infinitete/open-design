...
import { getAgentDef, SHIPPED_AGENT_DEFS } from '../../src/runtimes/registry.js';

const fsTest = process.platform === 'win32' ? test.skip : test;
const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../../../..');

// The retired ACP-backed agent runtime must stay out of the shipped registry,
// while the local agents that remain (codex, claude, opencode, …) keep their
// definitions so removal cannot silently widen.
test('shipped agent registry excludes the retired runtime but keeps local agents', () => {
  assert.equal(SHIPPED_AGENT_DEFS.some((agent) => agent.id === 'amr'), false);
  assert.equal(getAgentDef('amr'), undefined);
  assert.equal(SHIPPED_AGENT_DEFS.some((agent) => agent.id === 'codex'), true);
  assert.equal(SHIPPED_AGENT_DEFS.some((agent) => agent.id === 'claude'), true);
  assert.equal(SHIPPED_AGENT_DEFS.some((agent) => agent.id === 'opencode'), true);
  assert.equal(SHIPPED_AGENT_DEFS.some((agent) => agent.id === 'byok-opencode'), true);
  assert.equal(SHIPPED_AGENT_DEFS.some((agent) => agent.id === 'kimi'), true);
});
