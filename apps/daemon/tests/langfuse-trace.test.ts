...
describe('readRunTelemetrySinkConfig', () => {
  // The retired cloud provider used to win completed-run telemetry whenever a
  // control key was present. After the retirement its credentials are inert:
  // the resolver must return the generic relay/direct sink instead.
  it('ignores retired cloud-provider credentials and uses the generic sink', () => {
    ...
    expect(JSON.stringify(cfg)).not.toContain('vela.example.test');
  });

  it('resolves the hosted relay for anonymous completed-run telemetry', () => {
    const cfg = readRunTelemetrySinkConfig({
  ...
  it('describes the effective priority winner without paths, queries, or credentials', () => {
    const sink = readRunTelemetrySinkConfig({
      OPEN_DESIGN_TELEMETRY_RELAY_URL:
    ...
    const diagnostic = describeRunTelemetrySink(sink);
