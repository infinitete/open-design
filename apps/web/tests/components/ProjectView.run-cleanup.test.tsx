...
describe('ProjectView daemon cleanup', () => {
  it('threads active project delete to FileWorkspace at final invocation', async () => {
    ...
    expect(onDeleteProject).toHaveBeenCalledWith(projectId);
  ...
  it('keeps a pending-prompt signal until its clear succeeds', async () => {
    ...
    await waitFor(() => expect(onClearPendingPrompt).toHaveBeenCalled());
  ...
  it('translates aborted plugin actions to stale', async () => {
    ...
    installGeneratedPluginFolder.mockRejectedValueOnce(new DOMException('History changed', 'AbortError'));
  ...
  it('ignores a superseded live run status when Send now starts a replacement run', async () => {
  ...
  it('persists phantom recovery as failed when no active run backs a running row', async () => {
