...
interface SlashCommand {
  id: string;
  // Visible label, e.g. `/mcp`. Shown in the popover row.
  label: string;
  ...
  insert: string;
  // i18n key of the short description shown next to the label. Omitted
  // for commands whose label + argument hint already say everything.
  descKey?: keyof Dict;
...
export const ChatComposer = forwardRef<ChatComposerHandle, Props>(
  function ChatComposer(
    ...
    const slashCommands = useMemo<SlashCommand[]>(() => {
      ...
      return list;
    }, [researchAvailable, enabledMcpServers, onOpenMcpSettings]);
    ...
    async function submit() {
      ...
      // never hits the agent — this is a local UX hook, not a model prompt.
  ...
  return (
    <div className="slash-popover" data-testid="slash-popover">
      <div className="slash-popover-head">
        <span>{t('chat.slashPopoverTitle')}</span>
        <span className="slash-popover-hint">{t('chat.slashPopoverHint')}</span>
      </div>
      {/* The rows live in their own scroll port, not directly in the
          height-capped popover column — see `.slash-popover-list`. Carrying
          `role="listbox"` down here also keeps non-option chrome out of
          the listbox, whose only children may be options. */}
      <div className="slash-popover-list" role="listbox" aria-label={t('chat.slashPopoverAria')}>
