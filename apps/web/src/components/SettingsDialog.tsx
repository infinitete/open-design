...
// `about` must not be listed either — it owns a nav item, and mapping it to
// General used to send deep links to the wrong section with the wrong nav item
// highlighted.
...
export function SettingsDialog({
  ...
  // plan badges must consume it projected onto the selected workspace. See
  ...
  // Dedicated non-optimistic paths (agent network) skip the next autosave
  // effect tick so we do not double-write through handleConfigPersist.
  ...
  const renderAgentModelConfig = (selected: AgentInfo) => {
    ...
    const modelsForSelect = selected.models?.map((model) =>
      model.id === 'default'
    ...
    // Undefined === allow, matching today's UX.
