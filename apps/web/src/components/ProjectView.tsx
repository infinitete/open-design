...
type ProjectChatSendMeta = ChatSendMeta & {
  ...
  /** Marks this send as the design-system AI enrichment pass so the daemon
   *  prevents concurrent enrichment runs in the same conversation. */
...
type ConversationMaterializationRecovery = null;
...
function projectEventToAgentEvent(evt: ProjectEvent): LiveArtifactEventItem['event'] | null {
  ...
  if (evt.type !== 'live_artifact_refresh') return null;
  return {
...
export function ProjectView({
  ...
  // Retain the recommendation entry for starter context and first-artifact hints.
  ...
  const currentBrandExtractionId = null;
  ...
  // Every full transcript read claims a generation before crossing the
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const materializationRecovery = null;
  ...
  useEffect(() => {
    ...
    const reloadingCurrentConversation =
      ...
      && messagesAuthorityKeyRef.current === 'current';
    ...
    (async () => {
      try {
        ...
        if (cancelled || messagesRequestGenerationRef.current !== messagesRequestGeneration) return;
        setMessages((current) =>
          preservingLiveConversation
  ...
  const updateMessageById = useCallback(
    (
      ...
      if (!previous) return;
      const saved = updater(previous);
      const next = messagesRef.current.map((message) =>
        message.id === messageId ? saved : message
      );
      messagesRef.current = next;
      setMessages((current) => current.map((message) =>
        message.id === messageId ? saved : message
      ));
      // Keep React functional updaters pure. Persistence is derived once from
  ...
  }, [activeConversationId, refreshPreviewComments, projectEventsSseConnected]);
  ...
  useEffect(() => {
    ...
    const attachRecoverableRuns = async () => {
      ...
      for (const message of messages) {
        ...
        if (cancelled || !recoveryIsCurrent()) {
        ...
        void reattachDaemonRun({
          ...
          handlers: {
            ...
            onError: async (err) => {
              ...
              // Hand the active slot back before publishing a recovery tick;
              // the latest-issued token remains recorded, so any newer
              // attempt still invalidates late callbacks from this one.
  ...
  const handleSend = useCallback(
    async (
      ...
      const clearTouchedFilePaths = () => {
        pendingWrites.clear();
        touchedFilePaths.clear();
      };
      const provenTouchedFiles = () => [...touchedFilePaths]
      ...
      const updateAssistant = (updater: (prev: ChatMessage) => ChatMessage) => {
        if (!ownsRun()) return;
        const currentAssistant = messagesRef.current.find((message) => message.id === assistantId)
          ?? latestAssistantMsg;
        const updated = updater(currentAssistant);
        if (!ownsRun()) return;
        latestAssistantMsg = updated;
        const publish = (current: ChatMessage[]) => {
          if (!ownsRun()) return current;
          const found = current.some((message) => message.id === assistantId);
          if (found) {
            return current.map((message) => message.id === assistantId ? updated : message);
          }
          ...
          const userAlreadyPresent = current.some((message) => message.id === userMsg.id);
          return [
            ...current,
        ...
        const nextMessages = publish(messagesRef.current);
        ...
        if (ev.kind === 'tool_result') {
          ...
          if (filePath) {
            ...
            if (!ev.isError) {
              ...
              const immediateTouchedFiles = provenTouchedFiles();
      ...
      const cancelOwnedTextBuffer = (flushPending = false) => {
        if (flushPending) textBuffer.flush();
        textBuffer.cancel();
        if (sendTextBufferRef.current === textBuffer) sendTextBufferRef.current = null;
      };
      const handlers = {
        ...
        onDone: (fullText = '') => {
          ...
          if (!ownsRun()) {
            cancelOwnedTextBuffer();
            clearTouchedFilePaths();
            return;
          }
          cancelOwnedTextBuffer(true);
          for (const ev of parser.flush()) {
            if (ev.type === 'artifact:end') {
              parsedArtifact = parsedArtifact
                ? { ...parsedArtifact, html: ev.fullContent }
          ...
          if (emptyApiResponse) {
            ...
            onProjectsRefresh();
            clearTouchedFilePaths();
            releaseConversationOperation(runConversationId, operationToken);
            return;
          }
          const endedAt = Date.now();
                ...
                if (!latestRunStatus || isActiveRunStatus(latestRunStatus.status)) {
                  ...
                  if (ownsRun()) {
                    ...
                    updateAssistant((prev) => {
                      ...
                      if (
                        !recovered.producedFiles?.length
                        && (recovered.content.trim().length > 0 || (recovered.events?.length ?? 0) > 0)
                      ) {
      ...
      if (config.mode === 'daemon') {
        ...
        void streamViaDaemon({
          ...
          locale,
          ...(meta?.designSystemEnrichment ? { designSystemEnrichment: true } : {}),
          ...
          onRunStatus: (runStatus) => {
            ...
            if (isTerminalRunStatus(runStatus)) {
              ...
              scheduleConversationMessageRefresh(runConversationId);
              if (runStatus !== 'succeeded') clearTouchedFilePaths();
            }
          },
          onRunEventId: (lastRunEventId) => {
            if (!ownsRun()) return;
  ...
  const handleComposerSend = useCallback(
    async (
      ...
      const accepted = await handleSend(prompt, attachments, commentAttachments, meta);
      return accepted ? undefined : 'restore-draft';
    },
    [handleSend],
  );
    ...
    async (
      ...
      if (commentAttachments.length === 0) {
        const queued = uploaded.length > 0
          ? await handleSend('', uploaded, [], { queueOnly: true })
          : false;
        return {
  ...
  const handleNewConversation = useCallback(async () => {
    ...
    try {
      ...
      if (conversationCreationGenerationRef.current === creationGeneration) {
...
/** Local persisted delivery evidence; the legacy field name remains DB-compatible. */
