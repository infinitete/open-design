...
describe("open-design host contract", () => {
  ...
  it("ignores unknown bridge keys so a stale host build still validates", () => {
    // The updater namespace is gone from the contract, but a host build that
    // still injects one must keep working: validation checks the capabilities
    // it knows rather than rejecting anything extra.
    expect(isOpenDesignHostBridge({
      ...createMockOpenDesignHost(),
      updater: { check: async () => ({ ok: true }) },
    })).toBe(true);
  });
