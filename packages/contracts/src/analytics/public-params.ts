...
// behavioural funnel by: `byok` is the user's own key (web-only visibility —
// see the daemon note in `AnalyticsConfigureGlobals`), `local_cli` is a
// detected local coding CLI.
...
export interface AnalyticsConfigureGlobals {
  ...
  // only layer that knows whether the run it launched used its saved key.
  runtime_type: TrackingRuntimeType;
  // Per-path "reached a runnable/usable state" flags. Unlike `configure_type`
  // — a single priority cascade (both > local_cli > byok) where a value
  // masks the lower-priority paths — these are INDEPENDENT booleans, so
  ...
  // restated here only to give the runnable pair a symmetric shape.
  //
  ...
  // `settings_byok_test_result` with `result: 'success'`.
